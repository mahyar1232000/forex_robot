...
Supports live/backtest modes, dynamic parameter optimization,
and integrates dynamic risk management, a circuit breaker, and performance monitoring.
"""
...
class AdvancedTradingBot:
    ...
    def cleanup_legacy_files(self):
        for f in ['deepseek_model.joblib', 'scaler.joblib']:
            try:
                if os.path.exists(f):
                    os.remove(f)
    ...
    def load_or_create_model(self):
        try:
            if os.path.exists('deepseek_model.joblib') and os.path.exists('scaler.joblib'):
                self.model = load('deepseek_model.joblib')
    ...
    def create_new_model(self):
        try:
            ...
            dump(self.model, 'deepseek_model.joblib')
            dump(self.scaler, 'scaler.joblib')
            logger.info("Successfully created and trained initial model")
        except Exception as e:
    ...
    def detect_market_regime(self, df: pd.DataFrame) -> str:
        try:
            latest = df.iloc[-1]
            atr_ma = df['ATR'].rolling(window=50).mean().iloc[-1]
    ...
    def get_market_data(self) -> pd.DataFrame:
        try:
            ...
            if rates is None or len(rates) == 0:
                ...
                return pd.DataFrame()
            return self.process_data(rates)
        except Exception as e:
    ...
    def analyze_market(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            confidence = float(probabilities.max(initial=0))
            ...
            return {
                ...
                'time': df.iloc[-1]['time'],
                'stop_loss': df.iloc[-1]['ATR'] * self.get_atr_multiplier(self.detect_market_regime(df)),
    ...
    def run_backtest(self):
        ...
        for self.current_index in range(200, len(self.historical_data)):
            ...
            if signal['confidence'] >= self.config.get('min_confidence', 50):
